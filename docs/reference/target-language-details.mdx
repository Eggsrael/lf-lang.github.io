...
* `ctx.get_tag(&self) -> EventTag` Returns the tag at which the reaction executes.

* `ctx.get_start_time(&self) -> Instant` Returns the start time of the execution of this program.

* `ctx.get_physical_time(&self) -> Instant` Returns the current Physical time.

* `ctx.get_logical_time(&self) -> Instant` Returns the current Logical time.

* `ctx.get_elapsed_logical_time(&self) -> Duration` Returns the elapsed Logical time is frozen during the execution of a reactor.

* `ctx.get_elapsed_physical_time(&self) -> Duration` Returns the amount of physical time elapsed since the start of the program.
...
```rust
ctx.schedule_with_v(act, Some(30), Asap);
```

you can get the value from another reaction like so:

```rust
if let Some(value) = ctx.get(act) {
...
`ctx.request_stop(&self, offset: Offset)`
```lf-rs 
reactor shutdown { 
  reaction(startup) {=
...
For more information please look towards the reactor_rt crate, [runtime crate](https://lf-lang.github.io/reactor-rs/reactor_rt/index.html).
